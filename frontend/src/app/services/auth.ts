...
export class AuthService {
  ...
  private connectionSubject = new BehaviorSubject<string | null>(localStorage.getItem('access_token'));
    ...
    return this.http.post<any>(this.apiUrl, credentials).pipe(
      tap(response => {
        ...
        if (response && response.access_token) {
          ...
          this.connectionSubject.next(token);
                  }
      }),
      // 3. Capturamos errores para actualizar el estado de conexión
      catchError(error => {
        this.connectionSubject.next(''); // La API falló
